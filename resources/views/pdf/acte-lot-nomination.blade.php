...
<head>
    <meta charset="UTF-8">
    <title>{{ $typeActe->label() }} — Lot</title>
    <style>
...
<div class="page">
    ...
    <div class="objet">
        <strong>OBJET :</strong> Nomination collective — un seul acte, un seul circuit.
        Prise d'effet au <strong>{{ $lot->date_debut?->format('d/m/Y') }}</strong>.
    </div>

    <p class="corps">
        Le Directeur Général de l'ARTF décide de nommer les agents ci-dessous
        aux postes et structures indiqués.
    </p>
    ...
    <table>
        ...
        <tbody>
        @foreach($lot->nominations as $i => $ligne)
            <tr>
                <td>{{ $i + 1 }}</td>
